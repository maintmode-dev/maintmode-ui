...
/**
 * Cache key for the mentions picker — a DIFFERENT first segment from
 ...
 * this hook's UNFILTERED list, guests included. The approver combobox would
 * then start offering people who cannot approve. Passing `roles: []` is not a
 * fix either: `[].join(",") === ""` diverges only by accident, and the request
 * would go out with no roles at all.
 *
