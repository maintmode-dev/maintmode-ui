...
| ----------- | ---------------------------------------------- | -------------------------------------- | ------- | --------------------------- |
| `resources` | календарь: фильтр по ресурсам (боковая панель) | нет ключа в событии `CalendarEventDto` | RUK-256 | `maintenance-mapper.ts:236` |

**Поле живёт на `CalendarEvent`, а не на `Maintenance`** (RUK-258). Тот, кто
возьмёт RUK-256, правит доменный тип `CalendarEvent` в
[`maintenance.ts`](../src/domain/maintenance/maintenance.ts) — поле там уже
объявлено, менять тип не придётся, только заполнить его в маппере.
...
именно поэтому тип-уровневая сверка её не ловит (SPEC §4.2). Следствие видно в
проде: [`calendar-sidebar.tsx:62`](../src/features/calendar/calendar-sidebar.tsx)
строит фильтр через `resourceOptions(items)`, а `items` приходят из маппера с
`resources: []` — фильтр мёртв, и юнит-тесты зелёные, потому что гоняются на
рукописных фикстурах с непустыми `resources` (RUK-256).
