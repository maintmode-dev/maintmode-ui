...
| ---------------- | ---------------------------------------------- | -------------------------------------- | ------- | --------------------------- |
| `resources`      | календарь: фильтр по ресурсам (боковая панель) | нет ключа в событии `CalendarEventDto` | RUK-256 | `maintenance-mapper.ts:229` |
| `notify_targets` | календарь: показ каналов оповещения            | нет ключа в событии `CalendarEventDto` | —       | `maintenance-mapper.ts:230` |
| `steps`          | календарь: превью шагов без перехода в деталь  | нет ключа в событии `CalendarEventDto` | —       | `maintenance-mapper.ts:231` |
